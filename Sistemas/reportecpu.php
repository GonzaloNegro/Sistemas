...
<body>
        ...
        <div id="mostrar_reporte" style="width: 97%; margin-left: 20px; display: block;">
					...
					if($seleccion=='AREA'){
						...
						$consultar=mysqli_query($datos_base, "SELECT a.AREA, count(*) as TOTAL from inventario i left join area a on i.ID_AREA=a.ID_AREA
						group by a.AREA");
										...
										if ($listar['AREA']== 0) {
											...
											$nombre = $listar['AREA'];
													...
													"
														...
														<td class='text-center text-nowrap'><a class='btn btn-sm btn-outline-primary'  class=mod><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='currentcolor' margin='5' class='bi bi-eye' viewBox='0 0 16 16'>
                    ...
                    if ($seleccion=='ESTADO') {
						...
						<th class='cabecera'><p>TOTAL</p></th>
						<th class='cabecera' width=65px><p>ACCIÓN</p></th>
						</tr>
						</thead>";
						$consultar=mysqli_query($datos_base, "SELECT e.ESTADO, count(*) as TOTAL from inventario i left join estado_ws e on i.ID_ESTADOWS=e.ID_ESTADOWS
					    group by e.ESTADO");
									while($listar = mysqli_fetch_array($consultar))
									{
										if ($listar['ESTADO']== 0) {
					...
					if ($seleccion=='PROVEEDOR')
					{
						...
						$consultar=mysqli_query($datos_base, "SELECT e.PROVEEDOR, count(*) as TOTAL from inventario i left join proveedor e on i.ID_PROVEEDOR=e.ID_PROVEEDOR
						group by e.proveedor");
								...
								if ($seleccion=='SO')
								{
									...
									$consultar=mysqli_query($datos_base, "SELECT s.SIST_OP, count(*) as TOTAL from inventario i left join so s on i.ID_SO=s.ID_SO
									group by s.SIST_OP");
												while($listar = mysqli_fetch_array($consultar))
												{
													if ($listar['SIST_OP']== 0) {
