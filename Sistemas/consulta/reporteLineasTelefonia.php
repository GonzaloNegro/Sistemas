...
<html>
<head>
	<title>REPORTE LÍNEAS TELEFÓNICAS</title><meta charset="utf-8">
...
<section id="consulta">
		<div id="titulo" style="display: flex; flex-direction: column; gap: 2px;">
            ...
            <h1 class="subtitulo" style="margin-top:5px !important;">DETALLE POR LÍNEAS</h1>
            <h1 class="sbtRep" style="padding:2px;">REPORTE DE GASTOS DE TELEFONÍA MÓVIL</h1>
            <h1 style="padding:2px;" class="sbtRep" id="subtitulo" >DETALLE POR LÍNEAS</h1>
		</div>
        <form method="POST" id="form_filtro" action="./reporteLineasTelefonia.php" class="contFilter--name">
            <div class="filtros">
                <div class="filtros-listado">
                    ...
                    <style type="text/css" media="print">
                              @page {
                                /* pone en horizontal la pagina a imprimir*/
                                    size: landscape;
                                }
                    </style>
                    <script>
                ...
                if(isset($_POST['busqueda'])){
                    ...
                    //Personal
                    $queryy="SELECT m.ID_NOMBREPLAN,n.NOMBREPLAN AS nomPlanGr, p.PLAN AS planGr, pr.PROVEEDOR, n.MONTO
                    FROM movilinea m
                    INNER JOIN nombreplan n ON n.ID_NOMBREPLAN = m.ID_NOMBREPLAN
                    INNER JOIN plan p ON p.ID_PLAN = n.ID_PLAN
                    INNER JOIN proveedor pr on n.ID_PROVEEDOR=pr.ID_PROVEEDOR
                ...
                <h4 class='ind prov' style='margin-top: 2px; margin-bottom: 2px; font-size: 25px;font-weight: bold; text-decoration: underline;'>CLARO</h4>
                    ...
                    <div class='col linea'><p style='font-size:22px;text-align:left;text-decoration:underline;'></p></div>
                    <!--<div class='col'><p style='font-size:22px;text-decoration:underline;'>LINEA</p></div>-->
                    <div class='col usuario'><p style='font-size:22px;text-decoration:underline;'>USUARIO</p></div>
                    <div class='col area'><p style='font-size:22px;text-decoration:underline;'>AREA</p></div>
                    <div class='col costo'><p style='font-size:22px;text-decoration:underline;'>COSTO</p></div>
                    </div>";
                    //Bandera para indicar si el div de lineas pertenece a la primera pagina
                    $primeraPag = true;
                    $primerClaro=true;
                    $primerPersonal=true;
                    //Contador para indicar la cantidad de lineas que lleva el div
                    $contador=0;
                        while($rowSqli = $sqliCla->fetch_assoc()) {
                            $idNombrePlan=$rowSqli['ID_NOMBREPLAN'];
                            if ($primerClaro==false) {
                                echo "
                            ...
                            echo "
                             ...
                             <div class='col'><p class='planRepLineas' style='text-align:left;'>".$rowSqli['nomPlanGr']." - ".$rowSqli['planGr'].": </p></div>
                             <div class='col'></div>
                             <div class='col'></div>
                             <div class='col'></div>
                             
                             </div>";
                             //Se van contando las lineas
                            $contador++;
                            //traemos las lineas del plan 
                             
                            $sqlClaro="SELECT m.ID_MOVILINEA, m.ID_LINEA, l.NRO, n.NOMBREPLAN, n.MONTO, u.NOMBRE, m.MONTOTOTAL, a.AREA
                            FROM movilinea m
                            ...
                            where m.ID_MOVILINEA = (
                                 SELECT MAX(t.ID_MOVILINEA)
                                 FROM movilinea t
                                 WHERE m.id_linea = t.id_linea
                                 and MONTH(t.FECHA) =  $mesFiltro)
                                 and m.ID_NOMBREPLAN=$idNombrePlan
                                 order by a.ID_AREA";
                           $sqlLineasCla = $datos_base->query($sqlClaro);
                            //Si el contador va 10 y la bandera esta en true corresponde a la primera pagina, crea un nuevo div con la clase page-break(esta en el css) para 
                            // que este div salte a la siguiente pagina
                           if ($contador==9 && $primeraPag==true) {
                                echo"<div class='page-break'>";
                                $contador=0;
                                $primeraPag=false;
                           }
                           //Si el contador va en 21 y la bandera esta en falso, se cierra el div actual y se abre uno nuevo. Se reinicia el contador
                           if ($contador==20 && $primeraPag==false) {
                            echo"</div>";
                            echo"<div class='page-break'>";
                            $contador=0;
                            $primeraPag=false;
                       }
                            while ($rowSqlC = $sqlLineasCla->fetch_assoc()) {
                                
                               echo "
                               <div class='row' style='width:100%;display:flex;flex-direction:row;gap:10px;justify-content: space-between;'>
                               <div class='col linea'><p style='font-size:18px;text-align:left;'>".$rowSqlC['NRO']."</p></div>
                               <div class='col usuario'><p style='color:green;font-weight:bold;'>".$rowSqlC['NOMBRE']."</p></div>
                               <div class='col area'><p style='color:green;font-weight:bold;'>".$rowSqlC['AREA']."</p></div>
                               <div class='col costo'><p class='number' style='color:green;font-weight:bold;'>$".$rowSqlC['MONTOTOTAL']."</p></div>
                               </div>";
                               $contador++;
                                if ($contador==9 && $primeraPag==true) {
                                    echo"<div class='page-break'>";
                ...
                echo"<div id=''  class='page-break'>
                    ...
                    <h4 class='ind prov' style='margin-top: 2px; margin-bottom: 2px; font-size: 25px; font-weight: bold; text-decoration: underline;'>PERSONAL</h4>
                        ...
                        <div class='col costo'><p style='font-size:22px;text-decoration:underline;'>COSTO</p></div>
                            ...
                            while($rowSqli = $sqliPer->fetch_assoc()) {
                                $idNombrePlan=$rowSqli['ID_NOMBREPLAN'];
                                if ($primerPersonal==false) {
                                    echo "
                                <div class='border-bottom'></div>";
                                }
                                else {
                                    $primerPersonal=false;
                                }
                                echo "
                                 <div class='row' style='width:100%;display:flex;flex-direction:row;gap:10px;justify-content: space-between; margin-bottom:5px !important;'>
                                 <div class='col'><p class='planRepLineas' style='text-align:left;'>".$rowSqli['nomPlanGr']." - ".$rowSqli['planGr'].": </p></div>
                                 <div class='col'></div>
                                 <div class='col'></div>
                                 <div class='col'></div>
                                ...
                                $sqlPersonal="SELECT m.ID_MOVILINEA, m.ID_LINEA, l.NRO, n.NOMBREPLAN, n.MONTO, u.NOMBRE, m.MONTOTOTAL, a.AREA
                                FROM movilinea m
                                LEFT JOIN linea l ON m.ID_LINEA = l.ID_LINEA 
                                LEFT JOIN nombreplan n ON n.ID_NOMBREPLAN = l.ID_NOMBREPLAN 
                                LEFT JOIN plan p ON p.ID_PLAN = n.ID_PLAN
                                ...
                                where m.ID_MOVILINEA = (
                                    ...
                                    and m.ID_NOMBREPLAN=$idNombrePlan 
                                    order by a.ID_AREA";
                                $sqlLineasPer = $datos_base->query($sqlPersonal);
                               ...
                               if ($contador==20 && $primeraPag==false) {
                                ...
                                while ($rowSqlP = $sqlLineasPer->fetch_assoc()) {
                                    echo "
                                    <div class='row' style='width:100%;display:flex;flex-direction:row;gap:10px;justify-content: space-between;'>
                                    <div class='col linea'><p style='font-size:18px;text-align:left;'>".$rowSqlP['NRO']."</p></div>
                                    <div class='col usuario'><p style='color:green;font-weight:bold;'>".$rowSqlP['NOMBRE']."</p></div>
                                    <div class='col area'><p style='color:green;font-weight:bold;'>".$rowSqlP['AREA']."</p></div>
                                    <div class='col costo'><p class='number' style='color:green;font-weight:bold;'>$".$rowSqlP['MONTOTOTAL']."</p></div>
                                    
                                    </div>";
                                    $contador++;
                            //     if ($contador==10 && $primeraPag==true) {
                            //         echo"<div class='page-break'>";
                            //         $contador=0;
                            //         $primeraPag=false;
                            //    }
                               if ($contador==20 && $primeraPag==false) {
                                ...
                                $primeraPag=false;
                                 ...
                                 <div class='col'><p style='font-size:18px;text-align:left;'></p></div>
                                 <div class='col'><p style='color:green;font-weight:bold;'></p></div>
                                 <div class='col'><p id='subtotal' class='number' style='font-weight:bold;'>SUBTOTAL PERSONAL:</p></div>
                                 <div class='col'><p id='subtotal' class='number' style='color:green;font-weight:bold;'>$$montoPersonal</p></div>
                                 </div>
                                 </div>";
                                 $contador++;
                                 if ($contador==20 && $primeraPag==false) {
                    ...
                    //Monto Total
                                 ...
                                 <div id='totalPlanes' class='row' style='width:100%;display:flex;flex-direction:row;gap:10px;justify-content: space-between; margin-top:50px; height: 50px;'>
                                    <div class='col plan'><p style='font-size:18px;text-align:left;'></p></div>
                                    <div class='col'><p style='color:green;font-weight:bold;'></p></div>
