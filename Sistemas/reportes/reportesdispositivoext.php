...
<body>
        ...
        <div id="mostrar_reporte" style="width: 97%; margin-left: 20px; display: block;">
		            ...
		            <div class="form-group row justify-content-between" style="margin: 10px; padding:10px;">
	                    <a id="vlv"  href="reporteperifericos.php" class="col-3 btn btn-primary " type="button"  value="VOLVER"><i class="fa-solid fa-arrow-left"></i></a>
...
if ($tipodisp == 'scanner') {
    ...
    if($seleccion =='AREA'){
        #CONSULTA SQL PARA OBTENER CANTIDAD DE EQUIPOIS POR AREA
            ...
            $fecha = date("Y-m-d");
            echo "
            <h1 id='titulo'>REPORTE DE SCANNER POR AREA</h1>
                                        ...
                                        if ($listar['AREA']== 0) {
                                            ...
                                            $id_area=$listar['ID_AREA'];
                                                    ...
                                                    "
                                                        ...
                                                        <td class='text-center text-nowrap' id='accion' style='width: 80px;'><a class='btn btn-sm btn-outline-primary' href='../consulta/detalleareaperif.php?Area=".$id_area."&Tipo=$tipodisp' class=mod><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='currentcolor' margin='5' class='bi bi-eye' viewBox='0 0 16 16'>
...
#SE FILTRA POR ESTADO E SCANNER
    if ($seleccion=='ESTADO')
    {
                        ...
                        $fecha = date("Y-m-d");
                            echo "
                                <h1 id='titulo'>REPORTE DE SCANNER POR ESTADO</h1>
                                <hr style='display: block;'>
                                <h4 id='ind' class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>TOTAL EQUIPOS: ".$total['TOTAL']."</h4>
                                <h4 class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>FECHA ACTUAL: ".$fecha."</h4>
                                ...
                                where p.TIPOP='SCANNER' group by e.ESTADO");
                                            while($listar = mysqli_fetch_array($consultar))
                                            {
                                                if ($listar['ESTADO']== 0) {
                                                    $nombre = 'SIN ESTADO';
    ...
    if ($seleccion=='PROVEEDOR')
        {
            ...
            $fecha = date("Y-m-d");
                echo "
                    <h1 id='titulo'>REPORTE DE SCANNERS POR PROVEEDOR</h1>
                    <hr style='display: block;'>
                    <h4 id='ind' class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>TOTAL EQUIPOS: ".$total['TOTAL']."</h4>
                    <h4 class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>FECHA ACTUAL: ".$fecha."</h4>
                    <table id='tablareporte' class='table table-striped table-hover' width=97%>
                    ...
                    where p.TIPOP='SCANNER' group by e.proveedor");
                                while($listar = mysqli_fetch_array($consultar))
                                {
        ...
        if($seleccion=='AREA'){
    		...
    		$fecha = date("Y-m-d");
    		echo "
    		<h1 id='titulo'>REPORTE DE DISPOSITIVOS POR AREA</h1>
    		<hr style='display: block;'>
    		<h4 id='ind' class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>TOTAL EQUIPOS: ".$total['TOTAL']."</h4>
    		<h4 class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>FECHA ACTUAL: ".$fecha."</h4>
    		<table id='tablareporte' class='table table-striped table-hover' width=97%>
    		    <thead style='border-bottom: solid 5px #073256 !important;'>
    				<tr>
                        ...
                        LEFT JOIN area a ON a.ID_AREA = u.ID_AREA 
                        LEFT JOIN reparticion r ON a.ID_REPA = r.ID_REPA 
                        where p.TIPOP!='SCANNER' and p.TIPOP!='MONITOR' and p.TIPOP!='IMPRESORA' group by a.AREA");
    								while($listar = mysqli_fetch_array($consultar))
    								{
    									if ($listar['AREA']== 0) {
    										$nombre = 'SIN AREA';
...
#ESTADO
    	if ($seleccion=='ESTADO')
    {
    					...
    					$fecha = date("Y-m-d");
    						echo "
    							<h1 id='titulo'>REPORTE DE DISPOSITIVOS POR ESTADO</h1>
    							<hr style='display: block;'>
    							<h4 id='ind' class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>TOTAL EQUIPOS: ".$total['TOTAL']."</h4>
    							<h4 class='indicadores' style='margin-top: 20px; margin-bottom: 20px;'>FECHA ACTUAL: ".$fecha."</h4>
    							<table id='tablareporte' class='table table-striped table-hover' width=97%>
    							...
    							$consultar=mysqli_query($datos_base, "SELECT e.ESTADO, p.ID_ESTADOWS, count(*) as TOTAL from periferico p left join estado_ws e on p.ID_ESTADOWS=e.ID_ESTADOWS
    							where p.TIPOP!='SCANNER' and p.TIPOP!='MONITOR' and p.TIPOP!='IMPRESORA' group by e.ESTADO");
    										while($listar = mysqli_fetch_array($consultar))
    										{
    											if ($listar['ESTADO']== 0) {
    												$nombre = 'SIN ESTADO';
    ...
    if ($seleccion=='PROVEEDOR' )
        {
            ...
            $fecha = date("Y-m-d");
                echo "
                    ...
                    $consultar=mysqli_query($datos_base, "SELECT e.PROVEEDOR, p.ID_PROVEEDOR, count(*) as TOTAL from periferico p left join proveedor e on p.ID_PROVEEDOR=e.ID_PROVEEDOR
                    where p.TIPOP!='SCANNER' and p.TIPOP!='MONITOR' and p.TIPOP!='IMPRESORA' group by e.proveedor");
                                while($listar = mysqli_fetch_array($consultar))
                                {
                                    if ($listar['PROVEEDOR']== 0) {
                                        $nombre = 'SIN PROVEEDOR';
