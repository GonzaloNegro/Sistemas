        ...
        <div id="mostrar_reporte" style="width: 97%; margin-left: 20px; display: block;">
			        ...
			        <?php
                        ...
                        if ($proveedor==0) {
                            ...
                            $tit=$consultit['PROVEEDOR'];
                        }
                        if ($tipo=='otros') {
							$conttotal=mysqli_query($datos_base, "SELECT COUNT(*) as TOTAL from periferico p WHERE p.TIPOP!='MONITOR' and p.TIPOP!='IMPRESORA' AND p.TIPOP!='SCANNER' and p.ID_PROVEEDOR=$proveedor");
                    ...
                    echo "
							...
							$conttotal=mysqli_query($datos_base, "SELECT COUNT(*) as TOTAL from periferico p where p.TIPOP='$tipo' and p.ID_PROVEEDOR=$proveedor");
			            $total = mysqli_fetch_array($conttotal);
						$fecha = date("Y-m-d");
